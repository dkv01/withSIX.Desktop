// <copyright company="SIX Networks GmbH" file="W6DownloaderExtensions.cs">
...
namespace withSIX.Core.Extensions
{
    ...
    {
        public static Task<T> GetJson<T>(this Uri uri, CancellationToken ct = default(CancellationToken),
                string token = null)
            => uri.GetJson<T>(ct, client => Setup(client, uri, token));

        public static Task<string> GetJsonText(this Uri uri, CancellationToken ct = default(CancellationToken),
                string token = null)
            => uri.GetJsonText(ct, client => Setup(client, uri, token));

        public static Task<string> PostJson(this object model, Uri uri,
